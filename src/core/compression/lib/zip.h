 ...
 */
extern ZIP_EXPORT struct zip_t *zip_open(const wchar_t *zipname, int level,
                                         char mode);
 ...
 * @return the return code - 0 on success, negative number (< 0) on error.
 */
extern ZIP_EXPORT int zip_entry_fwrite(struct zip_t *zip, const wchar_t *filename);
 ...
 * @return the return code - 0 on success, negative number (< 0) on error.
 */
extern ZIP_EXPORT int zip_entry_fread(struct zip_t *zip, const wchar_t *filename);
 ...
 * @return the return code - 0 on success, negative number (< 0) on error.
 */
extern ZIP_EXPORT int zip_create(const wchar_t *zipname, const wchar_t *filenames[],
                                 size_t len);
 ...
 * @return the return code - 0 on success, negative number (< 0) on error.
 */
extern ZIP_EXPORT int zip_extract(const wchar_t *zipname, const wchar_t *dir,
                                  int (*on_extract_entry)(const wchar_t *filename,
                                                          void *arg),
                                  void *arg);
int zip_archive_extract(mz_zip_archive *zip_archive, const wchar_t *dir,
                               int (*on_extract)(const wchar_t *filename,
                                                 void *arg),
                               void *arg);
/** @} */

#define zip_lasterror(zip) (mz_zip_get_error_string(mz_zip_get_last_error(&zip->archive)))

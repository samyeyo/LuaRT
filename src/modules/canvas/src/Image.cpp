...
LUA_METHOD(Image, draw) {
    ...
    img->d->BeginDraw();
    img->d->DCRender->DrawBitmap(img->Bitmap, D2D1::RectF(x, y, x+size.width*ratio, y+size.height*ratio), luaL_optnumber(L, 5, 1.0f), D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, NULL);
    return 0;
}
...
LUA_METHOD(Image, drawrect) {
    ...
    img->d->BeginDraw();
    img->d->DCRender->DrawBitmap(img->Bitmap, D2D1::RectF(static_cast<FLOAT>(luaL_checknumber(L, 2)), static_cast<FLOAT>(luaL_checknumber(L, 3)), static_cast<FLOAT>(luaL_checknumber(L, 4)), static_cast<FLOAT>(luaL_checknumber(L, 5))), luaL_optnumber(L, 6, 1.0f), D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, NULL);
    return 0;
}
